...
from core.models import Paciente, Producto, Reserva
...
from .forms import ProductoForm, PacienteForm, ReservaForm
...
def form_reserva(request):
    ...
    if request.method== 'POST':
        ...
        if formulario.is_valid():
            formulario.save()
            datos['mensaje'] = "Guardados Correctamente"
    return render(request, 'core/form_reserva.html',datos)

def form_mod_reserva(request, id):
    reserva = Reserva.objects.get(numero=id)
    datos = {
        'form': ReservaForm(instance=reserva)
    }
    if request.method== 'POST':
        formulario = ReservaForm(date=request.POST,instance=reserva)
        if formulario.is_valid():
            formulario.save()
            datos['mensaje'] = "Modificados Correctamente"
...
def reserva (request):
    ...
    return render(request, 'core/reserva.html', datos)